...
Nitivo uses the modular monolith in ADR 0001. This optional setup is not the current architecture: inspect backend/ and existing conventions before proposing a packages root. Scope the configuration to an actual requested need; do not reorganize the application merely to fit the example. Adapt paths, tsconfig and checks to the existing workspace.
...
For the vocabulary (deep module, interface, seam, depth), read and apply [codebase-design](../codebase-design/SKILL.md) and use its language throughout.
...
Four principles implemented by five forbidden rules at `error` severity:
...
The template also enforces that production files do not import test fixtures.
...
Create a temporary `<packages-root>/example/` for validation; retain it in a commit only if the requested setup includes a permanent example:
...
Remove only the temporary example you created after validation unless the user requested retaining it.
...
Then add a **context pointer** to it from the repo's agent-instructions file (`AGENTS.md` if present, else `CLAUDE.md`, creating `AGENTS.md` if neither exists). One line is enough, e.g. point to the generated packages README and say to read it before adding or importing a package. This is what makes an agent discover the boundary rule instead of tripping over it.
