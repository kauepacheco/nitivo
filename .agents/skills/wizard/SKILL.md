...
The distributed template is inert; author stages only for the requested procedure. Its write_env helper stores raw single-line values: validate identifiers and ensure the application dotenv parser preserves the values, adapting serialization when needed. Never source the generated .env. Keep credential files ignored and private; publish secrets only to the explicitly intended repository.
